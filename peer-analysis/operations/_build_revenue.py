...
<style>
  ...
  *:focus-visible { outline:2px solid var(--ops-green); outline-offset:2px; border-radius:3px; }
  ...
  .yoy-cell { color:var(--ops-ink); font-weight:600; transition:transform 0.12s; }
  .yoy-cell:hover { transform:scale(1.05); z-index:2; box-shadow:0 0 0 1px var(--ops-green); }
  /* Row hover highlight in YoY matrix */
  .yoy-matrix tbody tr:hover td { box-shadow:inset 0 -2px 0 var(--ops-green); }
  .yoy-matrix tbody tr:hover td.peer-cell { background:rgba(45,80,22,0.08); }
  body.theme-dark .yoy-matrix tbody tr:hover td.peer-cell { background:rgba(34,197,94,0.12); }
  .yoy-matrix td.col-highlight { outline:2px solid rgba(245,158,11,0.5); outline-offset:-2px; }
  /* Cross-section peer highlight (main table ↔ YoY heatmap) */
  ...
  /* Top-3 정렬 medal: sortable 컬럼 정렬 시 favorites 제외 상위 3개 행에 🥇🥈🥉 */
  ...
  .tier-pills { display:flex; gap:8px; flex-wrap:wrap; align-items:center; margin:6px 0 8px 0; }
  ...
  @media print {
    ...
    .ops-head, .ops-foot, .year-bar, .rev-tab-bar, .action-bar, .theme-toggle, .anchor-nav, .trend-mode-toggle, .tier-pills { display:none !important; }
...
<header class="ops-head">
  <div class="ops-wrap ops-head-row">
    ...
    <nav class="ops-nav"><a href="dashboard.html">🎯 비교</a><a href="clubs/index.html">⛳ 클럽</a><a href="unit-economics.html">단위경제</a><a href="revenue.html" class="active">매출</a><a href="cost-hr.html">비용</a><a href="assets.html">시설</a><a href="risk.html">위험</a><a href="../../index.html" class="back">← 지도</a></nav>
  </div>
</header>
  ...
  <div class="ops-wrap">
    <div class="rev-panel active" id="panel-cmp">
      ...
      <div class="action-bar">
        ...
        <span class="row-count" id="rev-row-count" style="font-size:12px; color:var(--ops-muted); margin-left:auto;">13 / 13 표시</span>
      </div>
      <div class="tbl-card scroll-x">
        <table class="ops-tbl">
          <thead>
...
function applyCompScope() {
  document.querySelectorAll('#cmp-tbody tr, #yoy-matrix-body tr').forEach(tr => {
...
function applyRevTierFilter() {
  ...
  document.querySelectorAll('#cmp-tbody tr').forEach(tr => {
    ...
    const show = (revTierFilter === 'all') || (tier === revTierFilter);
    tr.classList.toggle('tier-hidden', !show);
    if (show) visible++;
  });
  document.querySelectorAll('#yoy-matrix-body tr').forEach(tr => {
    const tier = tr.dataset.tier;
    const show = (revTierFilter === 'all') || (tier === revTierFilter);
    tr.classList.toggle('tier-hidden', !show);
  });
  const rc = document.getElementById('rev-row-count');
...
function render(year){
  ...
  // Tertile thresholds across visible peer set (use full 13 for stability; null값 무시)
  function tertileBounds(arr) {
    const v = arr.filter(x => x !== null && x !== undefined && !isNaN(x)).sort((a,b) => a-b);
    if (v.length < 3) return [null, null];
    return [v[Math.floor(v.length/3)], v[Math.floor(2*v.length/3)]];
  }
  function tertileDot(value, lo, hi) {
    if (value === null || value === undefined || lo === null || hi === null) return '';
    if (value >= hi) return '<span class="tertile-dot dot-top" title="상위 1/3">●</span>';
    if (value >= lo) return '<span class="tertile-dot dot-mid" title="중위 1/3">●</span>';
    return '<span class="tertile-dot dot-bot" title="하위 1/3">●</span>';
  }
  const tbRev   = tertileBounds(rowData.map(r => r.rev));
  const tbGolf  = tertileBounds(rowData.map(r => r.golf));
  const tbCagr  = tertileBounds(rowData.map(r => r.cagr));
  const tbYoyG  = tertileBounds(rowData.map(r => r.yoyG));
  if (cmpSortState.key) {
    rowData.sort((a, b) => {
  ...
  const rows = rowData.map(r => {
    ...
    let rankCls = '';
    if (cmpNumericSort && !fav) {
      nonFavCount++;
      if (nonFavCount <= 3) rankCls = ` rank-${nonFavCount}`;
    }
    return `<tr data-tier="${d.tier}" class="${(fav ? 'is-fav' : '') + rankCls}">
      ...
      <td class="num">${fmtCAGR(cagr)}${tertileDot(r.cagr, tbCagr[0], tbCagr[1])}</td>
      <td class="num col-low-prio">${peakHtml}</td>
      <td class="num"><strong>${fmtBn(golf)}</strong>${tertileDot(golf, tbGolf[0], tbGolf[1])}</td>
      <td class="num col-low-prio">${fmtPct(share)}</td>
      <td class="num">${fmtYoY(yoyG)}${tertileDot(yoyG, tbYoyG[0], tbYoyG[1])}</td>
...
function renderYoYMatrix() {
  ...
  const html = PEER_ORDER.map(t => {
    ...
    const cells = yrSpans.map(([y0, y1], i) => {
      ...
      return `<td class="yoy-cell" data-col="${i}" style="background:${c.bg}; color:${c.fg};" title="FY${y0} → FY${y1}">${c.txt}</td>`;
    }).join('');
    const cagr = calcCAGR(d.yearly);
...
(function(){
  ...
  // Tier filter pills (Tab ①)
  document.querySelectorAll('#rev-tier-pills .tier-pill').forEach(p => {
    p.addEventListener('click', () => {
      document.querySelectorAll('#rev-tier-pills .tier-pill').forEach(x => x.classList.remove('active'));
      p.classList.add('active');
  ...
  // YoY matrix column hover highlight
  document.querySelectorAll('#yoy-matrix thead th[data-col]').forEach(th => {
    const col = th.dataset.col;
    th.addEventListener('mouseenter', () => {
      document.querySelectorAll(`#yoy-matrix-body td[data-col="${col}"]`).forEach(c => c.classList.add('col-highlight'));
    });
    th.addEventListener('mouseleave', () => {
      document.querySelectorAll(`#yoy-matrix-body td[data-col="${col}"]`).forEach(c => c.classList.remove('col-highlight'));
    });
  });
  // Cross-section peer highlight: main 비교 표 ↔ YoY 히트맵 hover sync
  ...
  function attachPeerHoverSync(containerId) {
    ...
    c.addEventListener('mouseover', (e) => {
      ...
      if (!ticker) return;
      ['cmp-tbody','yoy-matrix-body'].forEach(id => {
    ...
    c.addEventListener('mouseout', (e) => {
      const tr = e.target.closest('tr');
      if (!tr) return;
      const rel = e.relatedTarget && e.relatedTarget.closest && e.relatedTarget.closest('tr');
      if (rel && getTickerFromTr(rel) === getTickerFromTr(tr)) return;
      document.querySelectorAll('.peer-highlight').forEach(r => r.classList.remove('peer-highlight'));
    });
  }
  attachPeerHoverSync('cmp-tbody');
  attachPeerHoverSync('yoy-matrix-body');
  ...
  function loadRevState() {
    try {
      ...
      if (s.tier && ['all','pp','resort','twn'].includes(s.tier)) {
        revTierFilter = s.tier;
        document.querySelectorAll('#rev-tier-pills .tier-pill').forEach(p => p.classList.toggle('active', p.dataset.tier === s.tier));
      }
    } catch (e) {}
  }
  loadRevState();
  ...
  function readRevHash() {
    ...
    return { year: p.get('year'), tab: p.get('tab'), tier: p.get('tier') };
  }
  function syncRevHash() {
    ...
    if (revTierFilter && revTierFilter !== 'all') parts.push('tier=' + revTierFilter);
    const h = parts.join('&');
  ...
  if (fromHash) {
    ...
    if (fromHash.tier && ['all','pp','resort','twn'].includes(fromHash.tier)) {
      revTierFilter = fromHash.tier;
      document.querySelectorAll('#rev-tier-pills .tier-pill').forEach(p => p.classList.toggle('active', p.dataset.tier === revTierFilter));
    }
  }
  document.querySelectorAll('.year-btn, .rev-tab, .trend-mode-toggle button, #rev-tier-pills .tier-pill').forEach(el => {
