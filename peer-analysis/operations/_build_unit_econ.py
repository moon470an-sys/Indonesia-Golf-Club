...
<style>
  ...
  tr.tier-hidden, tr.search-hidden { display:none; }
  /* Top-3 정렬 medal: 사용자가 numeric 컬럼으로 정렬 시 상위 3개 행에 🥇🥈🥉 (favorites 제외 첫 3) */
  .ops-tbl tbody tr.rank-1 td:first-child,
  .ops-tbl tbody tr.rank-2 td:first-child,
  .ops-tbl tbody tr.rank-3 td:first-child { position:relative; padding-left:28px !important; }
  .ops-tbl tbody tr.rank-1 td:first-child::before,
  .ops-tbl tbody tr.rank-2 td:first-child::before,
  .ops-tbl tbody tr.rank-3 td:first-child::before {
    position:absolute; left:6px; top:50%; transform:translateY(-50%); font-size:13px; line-height:1;
  }
  .ops-tbl tbody tr.rank-1 td:first-child::before { content:'🥇'; }
  .ops-tbl tbody tr.rank-2 td:first-child::before { content:'🥈'; }
  .ops-tbl tbody tr.rank-3 td:first-child::before { content:'🥉'; }
  @media print {
    .ops-tbl tbody tr.rank-1 td:first-child::before,
    .ops-tbl tbody tr.rank-2 td:first-child::before,
    .ops-tbl tbody tr.rank-3 td:first-child::before { display:none; }
    .ops-tbl tbody tr.rank-1 td:first-child,
    .ops-tbl tbody tr.rank-2 td:first-child,
    .ops-tbl tbody tr.rank-3 td:first-child { padding-left:5px !important; }
  }
  ...
  /* Peer search input */
  .peer-search { padding:6px 10px; border:1px solid var(--ops-line); background:var(--ops-surface); border-radius:6px; font-size:12.5px; color:var(--ops-ink); min-width:180px; }
  .peer-search:focus { outline:none; border-color:var(--ops-green); box-shadow:0 0 0 3px rgba(45,80,22,0.12); }
  body.theme-dark .peer-search { background:#0f172a; }
  mark.q-match { background:rgba(245,158,11,0.45); color:inherit; padding:0 1px; border-radius:2px; }
  @media print { .peer-search { display:none !important; } }
  /* Theme toggle */
  ...
  /* Cross-section hover highlight */
  .ops-tbl tbody tr.peer-highlight td:first-child { background:rgba(245,158,11,0.18); }
  .ops-tbl tbody tr.tier-highlight td:first-child { box-shadow:inset 4px 0 0 #f59e0b; }
  body.theme-dark .ops-tbl tbody tr.peer-highlight td:first-child { background:rgba(245,158,11,0.28); }
  /* In-page anchor nav */
  ...
  /* Comparison scope pills (Direct / Segment / All) — 골프 매출 비중 기준 */
  ...
  .comp-note { font-size:11px; color:var(--ops-muted); margin:2px 0 6px; }
  /* Mobile column priority — hide less critical cols on narrow screens */
  @media (max-width: 720px) {
    .ops-tbl .col-low-prio { display:none; }
  }
  /* Density mode: essential — hide low-priority columns even on desktop */
  body.density-essential .ops-tbl .col-low-prio { display:none; }
  /* Density toggle UI (mimics view-toggle look) */
  .density-toggle { display:inline-flex; border:1px solid var(--ops-line); border-radius:6px; overflow:hidden; vertical-align:middle; }
  .density-toggle button { padding:6px 12px; border:none; background:var(--ops-surface); font-size:12px; font-weight:600; cursor:pointer; color:var(--ops-ink-soft); }
  .density-toggle button.active { background:var(--ops-green); color:white; }
  .density-toggle button + button { border-left:1px solid var(--ops-line); }
  body.theme-dark .density-toggle button:not(.active) { background:#0f172a; }
  @media print { .density-toggle { display:none !important; } }
...
<section class="ops-section" id="main-content">
  <div class="ops-wrap">
    <h2 class="anchor-target" id="sec-main" tabindex="-1" style="font-size:17px; margin:0 0 6px 0;">① 13-peer 단위 경제 — <span id="yr1">FY2025</span></h2>
    ...
    <div class="action-bar">
      <input type="search" class="peer-search" id="ue-search" placeholder="🔍 Peer 검색 (티커·그룹명)" aria-label="Peer 검색" autocomplete="off">
      <div class="density-toggle" role="group" aria-label="컬럼 밀도">
        <button data-density="all" class="active" title="모든 컬럼 표시 (15개)">전체</button>
        <button data-density="essential" title="핵심 컬럼만 (6개): Peer·홀·골프매출·CAGR·홀당매출·영업이익률">핵심</button>
      </div>
...
<section class="ops-section">
  <div class="ops-wrap">
    <h2 class="anchor-target" id="sec-leaderboard" style="font-size:17px; margin:0 0 14px 0;">② 홀당 매출 리더보드 — <span id="yr2">FY2025</span></h2>
...
<section class="ops-section">
  <div class="ops-wrap">
    <h2 class="anchor-target" id="sec-tier" style="font-size:17px; margin:0 0 14px 0;">③ Tier 평균 — <span id="yr3">FY2025</span></h2>
...
let tierFilter = 'all';
let searchQ = '';
// Comparison scope (Direct / Segment / All) — default Direct (3 peers with golf as main business)
...
function applyCompScope() {
  document.querySelectorAll('#main-tbody tr, #lb-tbody tr').forEach(tr => {
...
// Favorites — shared localStorage key with clubs/index (cross-page persistence)
...
function applyPeerSearchFilter() {
  ...
  document.querySelectorAll('#main-tbody tr').forEach(tr => {
    ...
    if (hit && !tr.classList.contains('tier-hidden')) visible++;
  });
  document.querySelectorAll('#lb-tbody tr').forEach(tr => {
    const text = tr.textContent.toLowerCase();
    tr.classList.toggle('search-hidden', !!q && !text.includes(q));
  });
  ['main-tbody','lb-tbody'].forEach(id => {
    const tb = document.getElementById(id);
    if (tb) markHits(tb, q);
  });
  const rc = document.getElementById('row-count');
  if (rc) rc.textContent = `${visible} / ${total} 표시`;
}
...
function renderMain(year){
  ...
  // Favorites stable-sort to top (shared localStorage 'clubs-favorites' with clubs/index)
  if (favorites.size > 0) {
    const favs = sorted.filter(r => favorites.has(r.t));
    const rest = sorted.filter(r => !favorites.has(r.t));
  ...
  // Track medal rank among non-favorite rows when sort is active
  const numericSort = !!sortState.key;
  let nonFavCount = 0;
  const html = sorted.map(r => {
    ...
    if (numericSort && !fav) {
      nonFavCount++;
      if (nonFavCount <= 3) rankCls = ` rank-${nonFavCount}`;
    }
    return `<tr class="${(fav ? 'is-fav' : '') + rankCls}">
      <td class="peer"><button class="fav-star${fav ? ' active' : ''}" data-ticker="${r.t}" title="${fav ? '즐겨찾기 해제' : '즐겨찾기 (피어 상단 고정)'}" aria-label="즐겨찾기 토글" aria-pressed="${fav}">${fav ? '★' : '☆'}</button><a href="clubs/${r.t.toLowerCase()}.html" style="color:var(--ops-ink); font-weight:700; text-decoration:none;">${r.t}</a><span class="peer-tag peer-tag-${r.d.tier}">${r.d.tier_label}</span></td>
  ...
  // Re-apply peer search filter (rows just got recreated)
  applyPeerSearchFilter();
  // Re-apply comparison scope (rows just got recreated)
  applyCompScope();
}
...
function render(year){
  ...
  // Re-apply peer search filter (leaderboard rows just got recreated)
  applyPeerSearchFilter();
  applyCompScope();
}
...
(function(){
  ...
  // Peer search input
  const ueSearch = document.getElementById('ue-search');
  if (ueSearch) {
    ueSearch.addEventListener('input', (e) => {
      searchQ = e.target.value;
      applyPeerSearchFilter();
    });
  }
  // Favorite-star click delegation (rebuilt rows preserve handler via parent delegation)
  ...
  // Density toggle (핵심/전체)
  let densityMode = 'all';
  function applyDensity(mode) {
    document.body.classList.toggle('density-essential', mode === 'essential');
    document.querySelectorAll('.density-toggle button').forEach(b => b.classList.toggle('active', b.dataset.density === mode));
  }
  document.querySelectorAll('.density-toggle button').forEach(b => {
    b.addEventListener('click', () => {
      densityMode = b.dataset.density;
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.target.matches('input,textarea,select')) {
      if (e.key === 'Escape' && e.target === ueSearch) {
        ueSearch.value = ''; searchQ = ''; applyPeerSearchFilter(); saveState(); syncHash(); e.target.blur();
      }
      return;
    }
  ...
  function saveState() {
    try {
      localStorage.setItem('ue-state', JSON.stringify({ year: currentYear, tier: tierFilter, sort: sortState, q: searchQ, density: densityMode }));
    } catch (e) {}
  }
  function loadState() {
    try {
      ...
      if (typeof s.q === 'string') {
        searchQ = s.q;
        const si = document.getElementById('ue-search');
        if (si) si.value = s.q;
      }
      if (s.density && ['all','essential'].includes(s.density)) {
        densityMode = s.density;
        applyDensity(densityMode);
      }
    } catch (e) {}
  }
  ...
  function syncHash() {
    ...
    if (densityMode && densityMode !== 'all') parts.push('density=' + densityMode);
    const h = parts.join('&');
  ...
  if (fromHash) {
    ...
    if (fromHash.q) {
      searchQ = fromHash.q;
      const si = document.getElementById('ue-search');
      if (si) si.value = fromHash.q;
    }
    if (fromHash.density && ['all','essential'].includes(fromHash.density)) {
      densityMode = fromHash.density;
      applyDensity(densityMode);
    }
  }
  // Save state on every user-driven change (year, tier, sort, search, density) + sync URL hash
  document.querySelectorAll('.year-btn, #tier-pills .tier-pill, #main-table .sortable, .density-toggle button').forEach(el => {
    el.addEventListener('click', () => setTimeout(() => { saveState(); syncHash(); }, 0));
  });
  if (ueSearch) ueSearch.addEventListener('input', () => setTimeout(() => { saveState(); syncHash(); }, 0));
  ...
  // Cross-section hover: main table row ↔ leaderboard + tier avg row
  function getTickerFromTr(tr) {
    const a = tr?.querySelector?.('a[href^="clubs/"]');
    return a ? a.textContent.trim() : null;
  }
  function getTierFromPeer(ticker) {
    return PEER_DATA[ticker]?.tier;
  }
  function attachHoverSync(tbodyId) {
    ...
    tb.addEventListener('mouseover', (e) => {
      ...
      if (!ticker) return;
      const tier = getTierFromPeer(ticker);
      // Highlight matching peer rows in main + leaderboard
      document.querySelectorAll('#main-tbody tr, #lb-tbody tr').forEach(r => {
        if (getTickerFromTr(r) === ticker) r.classList.add('peer-highlight');
      });
      // Highlight matching tier row in tier-tbody by tier label match
      if (tier) {
        document.querySelectorAll('#tier-tbody tr').forEach(r => {
          const lbl = r.querySelector('td:first-child')?.textContent || '';
          const match = (tier === 'pp' && lbl.includes('Pure-play')) || (tier === 'resort' && lbl.includes('Resort')) || (tier === 'twn' && lbl.includes('Township'));
          if (match) r.classList.add('tier-highlight');
        });
      }
    });
    tb.addEventListener('mouseout', () => {
      document.querySelectorAll('.peer-highlight').forEach(r => r.classList.remove('peer-highlight'));
      document.querySelectorAll('.tier-highlight').forEach(r => r.classList.remove('tier-highlight'));
    });
  }
  attachHoverSync('main-tbody');
  attachHoverSync('lb-tbody');
  render(currentYear);
})();
</script>
</body>
