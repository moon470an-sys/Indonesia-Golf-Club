...
## 라이브 사이트
**https://moon470an-sys.github.io/Indonesia-Golf-Club/?v=20260506g**

## 마지막 commit
`d98c721` — Tighten finance-tab guard: only show rows with numbers or ticker (149→50)

## 현재 상태 요약

### 데이터
- `data/golf_courses.json`: **149 코스** (운영중 137 · 휴장 9 · 불확실 3)
- 직전 백업: `data/golf_courses.backup.20260506_080716.json` (12 신규 코스 추가 직전)
- 그 이전 백업: `golf_courses.backup.20260506_070805.json` (68 finance corrections 직전)
- 그 이전: `golf_courses.backup.20260505_235620.json` (v4 머지 직전)
...
2. **Suvarna Jakarta 등 12 신규 코스의 가격(fees_2026_05) 수집**
   - 현재 모두 `fees_2026_05: null` 상태
   - 크롤러 v3 + 머지 v4 파이프라인 그대로 돌리면 됨
...
### 우선순위 낮음
5. **i18n 확장**: 현재 50+ key. 디테일 패널, 상세 데이터 헤더, 빈 상태 등이 아직 KR-only
6. **모바일 폴리쉬**: ≤480px 같은 매우 작은 화면 추가 점검
...
- 49개 운영중 코스에 가격 데이터 없음 (PDF/이미지 게시 한계)
